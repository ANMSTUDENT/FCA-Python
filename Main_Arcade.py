...
def draw_button(screen, text, rect, color, text_color, font, locked=False):
    """Draws a new, rounded-corner button with a slight lift effect."""
    border_radius = 12
    
    # Draw shadow/3D lift effect
    shadow_offset = 4
    shadow_rect = rect.move(0, shadow_offset)
    pygame.draw.rect(screen, BLACK, shadow_rect, 0, border_radius=border_radius)
    
    if locked:
        # Locked buttons use Dark Gray base
        pygame.draw.rect(screen, DARK_GRAY, rect, 0, border_radius=border_radius)
        pygame.draw.rect(screen, WHITE, rect, 2, border_radius=border_radius)
        text_surface = font.render("LOCKED", True, WHITE)
    else:
...
class ClownSplashMiniGame:
    ...
    def reset(self):
        ...
        self.MAX_WATER = self.WATER_MAX # Needed for original logic text
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_SPACE:
                ...
                # Reset all clowns visually when spray stops
...
class ArcadeManager:
    ...
    def _update_state(self, dt):
        ...
        if self.state == STATE_PRIZES:
            # Prize screen is mostly static, no movement update needed
            pass
        elif self.state in self.games:
    ...
    def _draw_menu(self):
        ...
        # 1b. Draw Carnival Stripes (Tent Top effect)
        stripe_width = 40
        for i in range(0, SCREEN_WIDTH + stripe_width, stripe_width):
            color = CARNIVAL_RED if (i // stripe_width) % 2 == 0 else CARNIVAL_YELLOW
        ...
        # Water Shooter button: Bright Green
        ...
        # Score: White Total Score, moved far left
        score_surf = self.header_font.render(f"TOTAL SCORE: {self.total_score}", True, WHITE)
        score_rect = score_surf.get_rect(topleft=(PLAY_AREA_MARGIN, SCREEN_HEIGHT - 50))
        self.screen.blit(score_surf, score_rect)

    def _draw_game_score(self):
        # UI Readout in top right corner (Game name + Score)
        score_surf = self.ui_font.render(f"SCORE: {self.total_score} | ESC to Menu", True, UI_BRIGHT_YELLOW) 
