...
UI_PLAYFUL = (150, 220, 255)  # readable light blue
...
# Prize ordering: top (expensive) -> bottom (cheaper), arranged row-first
PRIZES = [
    {"id": "car", "name": "New Car", "cost": 50000},          # top-left (very expensive)
    {"id": "scooter", "name": "Electric Scooter", "cost": 10000},  # top-right (very expensive)
    {"id": "sword", "name": "Shiny Sword", "cost": 12000},   # second row left
    {"id": "vr", "name": "VR Headset", "cost": 5000},        # second row right
    {"id": "robot", "name": "Mini Robot", "cost": 1500},     # third row left
    {"id": "nerf", "name": "Colored Nerf Gun", "cost": 3200},# third row right
    {"id": "lollipop", "name": "Giant Lollipop", "cost": 2500},# fourth row left
    {"id": "teddy", "name": "Teddy Bear", "cost": 500},      # fourth row right (cheapest)
]

# Score economy rebalancing
...
ACCESSIBILITY_OPTIONS = {
    ...
    "monochrome": False,
    "slow_game": False,
    "reticle_alt": False,  # toggles reticle color only
}

# --- Utilities ---
def atomic_write_json(path: str, data):
    # Atomic write JSON to avoid corruption
    try:
...
try:
    ...
    pass
pygame.display.set_caption("Jay's Carnival Arcade - Finalized")

# --- Sound manager ---
class SoundManager:
    # Manage simple audio calls (console placeholders)
    def __init__(self):
        self.is_spraying = False
        self._last_pop_time = 0.0
        self.muted = ACCESSIBILITY_OPTIONS["mute"]
    ...
    def play_pop(self):
        if self.muted:
            return
        now = time.time()
        if now - self._last_pop_time > 0.02:
...
def create_prize_icon_surf(icon_id: str, size: int = 64):
    # Create simple vector-like prize icons
    ...
    if icon_id == "teddy":
        ...
        # Improved robot icon: head, eyes, mouth, antenna, body panels
        ...
        # Improved scooter with deck, handlebar, and wheels
        ...
        # handlebar
        pygame.draw.line(surf, (60, 60, 60), (deck_x + 10, deck_y - 8), (deck_x + 10, deck_y - 28), 4)
        ...
        # Improved nerf-like blaster: barrel, body, muzzle
...
# --- Clown helper class ---
class Clown:
    # Clown visual helper with precomputed hair puffs
    def __init__(self, center_x, center_y, size=72):
        ...
        # PRECOMPUTE hair puff positions to avoid jitter between redraws
    ...
    def _generate_hair_puffs(self, seed=None):
        # Deterministic puff positions per clown
...
# --- Prize Screen (2 columns x 4 rows row-first) ---
class PrizeScreen:
    # Prize room UI and purchase flow
    ...
    def _compute_prize_positions(self):
        # Compute positions for 2x4 grid centered more tightly
        cols = 2
        rows = 4
        positions = []
        icon_w = 76  # smaller icons to fit better
        left_pad = 100
        right_pad = 100
            ...
            for pid, rect in self.prize_item_rects.items():
                if rect.collidepoint(mpos):
                    if self.unlocked.get(pid, False):
                        ...
                        buy_rect = pygame.Rect(SCREEN_WIDTH // 2 - 140, SCREEN_HEIGHT // 2 + 40, 120, 36)
                        cancel_rect = pygame.Rect(SCREEN_WIDTH // 2 + 20, SCREEN_HEIGHT // 2 + 40, 120, 36)
    ...
    def draw(self, total_score):
        ...
        for i, prize in enumerate(PRIZES):
            ...
            # dark brown pedestal only, positioned slightly above bottom of icon to look like shelf
            ...
            # center names & costs; move text slightly up to fit neatly in shelf box
            if unlocked:
                status_text = "OWNED"
            ...
            # nudge names upwards to sit "in" the shelf area
...
class DartPopGame:
    # Rotating reticle balloon pop mini-game
    ...
    def draw(self):
        ...
        # reticle color respects accessibility (color only)
...
class SwayObject(pygame.sprite.Sprite):
    # Simple vertical sway sprite used by power meter
...
class HoopShotGame:
    # Hoop shot: power-timing mini-game; net drawn under rim for correct visual alignment
    def __init__(self, screen, font, sound_manager):
        self.screen = screen
        self.font = font
        self.sound_manager = sound_manager
    ...
    def update(self, dt):
        ...
        # internal animation variable only
    ...
    def draw(self):
        ...
        # Compute rim_center so rim/backboard/net align together
        rim_center_x = self.hoop_center_x - 40
        hoop_center_x = rim_center_x
        hoop_center_y = self.hoop_center_y

        # draw backboard first (behind net)
        backboard_rect = (rim_center_x + 40, hoop_center_y - 75, 10, 150)
        pygame.draw.rect(self.screen, WHITE, backboard_rect)

        # Draw trapezoid net centered on rim_center_x and slightly moved up to tuck into rim
        net_top_width = 70
        net_bottom_width = 42
        net_height = 78
        net_top_y = hoop_center_y + 8   # moved slightly up to meet rim interior
        ...
        # Draw vertical strings (net) BEFORE the rim so the rim appears above the net (underlap)
        num_strings = 6
        ...
        # draw top line that sits under the rim interior
        pygame.draw.line(self.screen, WHITE, net_top_left, net_top_right, 2)

        # Draw rim (on top of net so it visually tucks the net below the rim)
...
def draw_clown_face_centered(surface, clown_size, hit, surface_size, hair_puffs: Optional[List[Tuple[int, int]]] = None):
    # Draw clown face ensuring upright smile alignment and correct nose rendering
...
# --- WaterGun & ClownSplash refinements ---
class WaterGun(pygame.sprite.Sprite):
    # Water gun: stream generation and particle effects with sway
    ...
    def update_stream(self, target_y, water_level_ratio, dt):
        ...
        if self.is_spraying:
            ...
            # create particles with initial sway phase
            for i in range(6):
                frac = random.random()
        ...
        for p in list(self.particles):
            ...
            p['y'] += p.get('vy', 30) * dt
            # sway using time + particle phase for side-to-side motion
...
class ClownSplashMiniGame:
    # Water shooting mini-game with logical cap and refill behavior and constant sway
    def __init__(self, screen, font, sound_manager):
        self.screen = screen
        self.font = font
        self.sound_manager = sound_manager
    ...
    def reset(self):
        ...
        # logical cap now 100 for easier percentage UX
        self.WATER_MAX = 100.0
        # start full
        self.water_level = self.WATER_MAX
        self.SPRAY_RATE = 10.0  # units per second (10% per second)
        # refill per tap (adjusted to 8)
    ...
    def draw(self):
        ...
        # reticle above stream (only for ClownSplash now) with constant sway
        ...
        try:
            ...
            # reticle sway based on global ticks
...
class WhackAMoleGame:
    ...
    def reset(self):
        ...
        self.rows = 4  # more vertical spacing possible, but we keep 2x3 earlier; preserve visual feel
        self.cols = 3
        self.hole_spacing_x = (self.PLAY_AREA_RECT.width) // (self.cols + 1)
        self.hole_spacing_y = (self.PLAY_AREA_RECT.height) // (self.rows + 1)
        self.targets = []
        for r in range(2):  # 2 rows visually (preserve earlier)
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.active_target and self.active_target.visible and self.active_target.rect.collidepoint(pos):
                score_change += self.active_target.whack()
                self.hits += 1
    ...
    def draw(self):
        ...
        self.screen.blit(hud, (SCREEN_WIDTH - hud.get_width() - 10, 10))
...
# --- Arcade Manager ---
class ArcadeManager:
    # Manager for menus, states, persistence, and settings
    def __init__(self):
    ...
    # Apply settings with smoothing target
    ...
    def _open_unlock_modal(self, state_id: int):
        ...
        purchase_rect = pygame.Rect(SCREEN_WIDTH // 2 - 140, SCREEN_HEIGHT // 2 + 40, 120, 25)
        cancel_rect = pygame.Rect(SCREEN_WIDTH // 2 + 20, SCREEN_HEIGHT // 2 + 40, 120, 25)
    ...
    def _draw_menu(self):
        ...
        draw_rounded_box(self.screen, overlay_rect, fill_color=None, border_color=CARNIVAL_YELLOW, border_thickness=5, radius=26)
        ...
        draw_rounded_box(self.screen, stats_button_rect, fill_color=(60, 60, 60), border_color=CARNIVAL_YELLOW, border_thickness=3, radius=12)
        bar_w = 8
        gap = 6
        bar_left = stats_button_rect.left + 10
        ...
        draw_rounded_box(self.screen, settings_button_rect, fill_color=(30, 30, 30), border_color=CARNIVAL_YELLOW, border_thickness=3, radius=12)
        ...
        draw_rounded_box(self.screen, prize_button_rect, fill_color=(45, 45, 60), border_color=CARNIVAL_YELLOW, border_thickness=3, radius=12)
    ...
    def _draw_settings_screen(self):
        ...
        label1 = self.ui_font.render("Slow Game (Reduce speed by 25%)", True, UI_PLAYFUL)
        ...
        label2 = self.ui_font.render("Monochrome (Desaturate - optimized)", True, UI_PLAYFUL)
        ...
        # Row 4: Reticle color (toggle color only)
        row_top += row_height + 10
        ...
        label4 = self.ui_font.render("Reticle Alt Color (Color only)", True, UI_PLAYFUL)
    ...
    def _apply_monochrome_filter(self):
        ...
        try:
            ...
            # Fallback: per-line sampling (slower but avoids big-block pixelation)
            w, h = surf.get_size()
